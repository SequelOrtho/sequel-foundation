...
- **Match sibling label typography, don't restyle the control.** `labelClassName` exists so a converted dropdown's label matches its neighbours (`Field` = `text-sm text-zinc-600 dark:text-zinc-400`; Project Hub `labelCls`; Audit Hub navy labels); pass only typography utilities, never the wrapper's own stack (block wrapper, inline label, `mt-0.5` control — see "Two primitives, one geometry" below). Sites whose neighbours already use the foundation default, and every `hideLabel` site, need nothing.
...
- **Two primitives, one geometry.** `Field` shipped as a flex column (`gap-1`) while `AdaptiveSelect`/`SearchCombobox` kept the hubs' inline-label + `mt-0.5` idiom (v0.7.1); the first form to mix them (Project Hub, Onboard a Resource) had its pickers ~5px lower than its inputs. Every label-bearing primitive now uses the native idiom (DESIGN-CONVENTIONS §3 "Field label geometry") — when adding one, copy that stack, never invent a gap.

## Lessons learned (Sep 2026, v0.12.1 fleet sweep)

- **Parallel gates on one container produce phantom test failures.** Running three hubs' `test` + `next build` at once starved the Acquisition Hub run (2 of 653 failed, no assertion output); the same suite passed 653/653 solo. Before treating a fleet-gate failure as real, re-run that repo's gate alone — and batch the large hubs (Acquisition, Project) one at a time.
- **`pin-foundation.sh` is idempotent across the fleet** — seven repos, one script invocation each, every lockfile landed on the tag SHA on the first try. When a release is a pure fix with no new exports, the pin PR is exactly two files; no shim or guide regeneration is needed.
