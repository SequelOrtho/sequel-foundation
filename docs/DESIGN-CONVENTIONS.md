...
- **Dropdowns over 12 items are fuzzy-searchable** (code: `ui/AdaptiveSelect.tsx`, `ui/SearchCombobox.tsx`, `ui/combo-match.ts`). A native `<select>` is fine up to 12 choices; past that it becomes a scroll-and-squint problem, so the field renders as the `SearchCombobox` — type-to-filter with fuzzy, ranked matching (prefix › word-prefix › substring › in-order subsequence › one typo/transposition), keyboard navigation, live result count, capped render with a "keep typing" hint. Render every dropdown whose list can grow (people, projects, entities, vendors, sites, taxonomies) as `<AdaptiveSelect>`: it counts the options and picks native vs. searchable at render time, so a picker that is small in one tenant and large in another obeys the rule in both without a code change; it takes `name` for form posts and `defaultValue` for uncontrolled forms exactly like a native select. Hard-coded enums (status, priority, role, month) stay native. `SEARCHABLE_SELECT_THRESHOLD` / `isSearchableSize` are the one definition of "more than 12" — never re-encode the number at a call site. (Owner ruling 2026-09-03, applied fleet-wide.)
